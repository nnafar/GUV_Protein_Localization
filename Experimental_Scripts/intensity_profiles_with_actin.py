...
At the current version the code always considers 3 channels available: M, S, A. 
Further modifications are needed to separate what is processed or not to include
one of the proteins.
...
4. The background noise is calculated and subtracted from the linear profiles.
   ...
   - For the actin channel:    Same process as in the septin channel. 
5. By averaging all the line profiles for all the angles a collective radial 
   profile is obtained for each vesicle. 
6. Membrane detection takes place, finding the absolute maximum and considering
...
txt_membrane = np.loadtxt('text_membrane_of_actin_region.txt')
txt_septin   = np.loadtxt('text_septin_of_actin_region.txt')
txt_actin    = np.loadtxt('text_actin_of_actin_region.txt')
...
detection_ch = pd.read_csv(r'Actin_region_composite_detected_vesicles_easy.csv')
...
colors          = ["black","yellow"]
nodes           = [0.0, 1.0]
my_cmap_yellow  = LinearSegmentedColormap.from_list("mycmap", list(zip(nodes, colors)))
...
def intensity_profile(vesicle_id, radius, xc, yc, num_angles, length_excess, dr, size_factor_mask, size_factor_view, txt_membrane, txt_septin, txt_actin, threshold_method):
    """
   ...
   size_factor_mask  : float 
    ...
    txt_actin : np.ndarray
        The text image of the actin channel.
    ...
    intensity_actin    = np.zeros((len(along_radius), num_angles))
    ...
    for i in range(len(along_radius)):
        for j in range((num_angles)):            
            ...
            if xc + length_excess*radius < image_dim_x and xc - length_excess*radius >= 0 and yc + length_excess*radius < image_dim_y and yc - length_excess*radius >= 0:
                ...
                intensity_actin[i,j]    = txt_actin[int(line_y[i,j]), int(line_x[i,j])]
            else:
                return
    ...
    vesicle_box_actin= txt_actin[int(yc-vesicle_box+move_down):int(yc+vesicle_box-move_up), int(xc-vesicle_box+move_right):int(xc+vesicle_box-move_left)]
    ...
    # Creating assistive mask for the protein channels in case of non-encapsulated clusters present in the picture
    mask_sept_assist = vesicle_box_sept > thresh_sept
    mask_actin_assist = vesicle_box_actin > thresh_actin
    ...
    # Protein masks are taken as the membrane mask adding the assistive corresponding protein mask in case of non-encapsulated protein clusters
    mask_sept  = np.where(mask_memb_fill + mask_sept_assist != 2, mask_memb_fill + mask_sept_assist, 1)
    mask_actin = np.where(mask_memb_fill + mask_actin_assist != 2, mask_memb_fill + mask_actin_assist, 1)
    ...
    background_actin    = np.mean(np.ma.array(vesicle_box_actin, mask=mask_actin))
    ...
    if plot_masks == 'True':
        ...
        ax2.set_axis_off()
        ax3.imshow(vesicle_box_actin , cmap = my_cmap_yellow)
        ax3.set_title('Actin')
        ax3.set_axis_off()
        plt.show()
        ...
        ax2.set_title('Septin')
        ax2.set_axis_off()
        ax3.imshow(mask_actin_assist , cmap = my_cmap_yellow)
        ax3.set_title('Actin')
        ax3.set_axis_off()
        plt.show()
        ...
        ax2.set_title('Septin')
        ax2.set_axis_off()
        ax3.imshow(mask_actin , cmap = my_cmap_yellow)
        ax3.set_title('Actin')
        ax3.set_axis_off()
        plt.show()    
    ...
    intensity_actin_corrected   = np.clip(intensity_actin - background_actin, 0, None) 
    ...
    mean_int_actin = np.average(intensity_actin_corrected, axis = 1) 
    ...
    localization_s = septin_bound/septin_centre
    
    actin_bound   = np.average(mean_int_actin[index_border_in:index_border_out])
    actin_centre  = np.average(mean_int_actin[0:int(1/4*radius)])  
    # might change to: actin_centre  = np.average(mean_int_actin[0:int(centre_relative_size*radius)])

    localization_a = actin_bound/actin_centre
    ...
    actin_angular    = np.average(intensity_actin_corrected[index_border_in:index_border_out,:], axis=0)  
    ...
    zoom_in_vesicle_actin = txt_actin[int(yc-zoom_in_box+move_down):int(yc+zoom_in_box-move_up), int(xc-zoom_in_box+move_right):int(xc+zoom_in_box-move_left)]
    ...
    if plot_final_results == 'True': 
        ...
        fig, axes = plt.subplot_mosaic("AB;AC;AD;AE", width_ratios = [3, 1])
        ...
        axes["A"].plot(along_radius, mean_int_actin, c = 'gold', label = 'actin')
        ...
        axes["A"].axvline(x = 1/4*radius[0], c ='green', label = 'central area border')
        ...
        axes["D"].imshow(zoom_in_vesicle_actin, cmap = my_cmap_yellow)
        axes["D"].set_axis_off()
        axes["E"].text(-0.2,0.7,"Background M:  " + str(round(background_membrane,4)), size = 10)
        axes["E"].text(-0.2,0.5,"Background S:   " + str(round(background_septin,4)), size = 10)
        axes["E"].text(-0.2,0.3,"Background A:   " + str(round(background_actin,4)), size = 10)
        axes["E"].text(-0.2,0.0,"Localization S :  " + str(round(localization_s,3)), size = 10)
        axes["E"].text(-0.2,-0.2,"Localization A :  " + str(round(localization_a,3)), size = 10)
        axes["E"].set_axis_off()
        ...
        axes["A"].plot(theta*180/np.pi, actin_angular, c = 'gold', label = 'actin')
        axes["A"].legend(loc="upper left")
    ...
    if plot_final_results_normalize == 'True': 
        ...
        axes["A"].plot(along_radius, mean_int_actin/max(mean_int_membr), c = 'gold', label = 'actin')
        axes["A"].axvline(x = along_radius[index_border_in], c = 'red', label = 'membrane borders')
        axes["A"].axvline(x = along_radius[index_border_out], c = 'red')
        axes["A"].axvline(x = 1/4*radius[0], c ='green', label = 'central area border')
        ...
        axes["A"].plot(theta*180/np.pi, actin_angular/max(membrane_angular), c = 'gold', label = 'actin')
        axes["A"].legend(loc="upper left")
        axes["A"].set(xlabel = '\u03B8 (deg)', ylabel = 'I / $I_{max}$')
        axes["B"].set_title("Membrane channel", fontsize = 10, pad =-1)
        axes["B"].imshow(zoom_in_vesicle_membrane, cmap = my_cmap_cyan)
        axes["B"].set_axis_off()
        axes["C"].set_title("Septin channel", fontsize = 10, pad =-1)
...
for vesicle_id in range(num_vesicles):
    intensity_profile(vesicle_id, radius[vesicle_id], xc[vesicle_id], yc[vesicle_id], num_angles, length_excess, dr, size_factor_mask, size_factor_view, txt_membrane, txt_septin, txt_actin, threshold_method ) 
